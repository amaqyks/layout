...
1. **标题保留序号与标点**：
   ...
   - **必须保持序列标题一致性**：如果一个章节内有多个并列子论点（例如“矛盾 1”、“矛盾 2”、“矛盾 3”、“矛盾 4”；或“① 胜任感”、“② 记录与再叙事”、“③ 情绪的面具”），**每一个子论点都必须统一加 \`###\` 标注为二级小标题**！绝对不可以只标其中一个而把后续同级子论点漏掉。

2. **引言与多行引用块规范 (\`> \`)**：
   - 全文开篇的总结段或金句导语，必须包裹在 \`> **「金句标题」**——内容\` 中。
   - 正文中的示例、案例、Prompt 模板、代码或多步骤要求等具有演示或深思性质的段落，整段使用 \`> \` 引用包裹。
   - **多行引用铁律**：如果引用包含多行文字或多个步骤（如 1. 2. 3. 4. 5.），**引用的每一行在行首必须全部加 \`> \` 标记**！绝对不可以只给第一行加 \`> \`，确保整套提示词或示例全貌被完整收纳在同一个引用框内。
